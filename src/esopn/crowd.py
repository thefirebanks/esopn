...
from .audio_utils import resample_audio, safe_normalize
...
class CrowdSoundGenerator:
    ...
    def _apply_envelope(
        ...
        if attack_samples < samples - release_samples:
            envelope[attack_samples : samples - release_samples] = sustain
...
class CrowdManager:
    ...
    def mix_with_commentary(
        ...
        Returns:
            Mixed audio array
        """
        crowd_wave = resample_audio(
            crowd_audio.audio, crowd_audio.sample_rate, commentary_sample_rate
        )

        if crowd_position == "before":
            # Crowd plays, then commentary
            padding = np.zeros(int(0.2 * commentary_sample_rate), dtype=np.float32)
            return np.concatenate([crowd_wave, padding, commentary_audio])

        elif crowd_position == "after":
            # Commentary plays, then crowd
            padding = np.zeros(int(0.2 * commentary_sample_rate), dtype=np.float32)
            return np.concatenate([commentary_audio, padding, crowd_wave])

        else:  # "under" - mix underneath commentary
            # Ensure same length
            commentary_len = len(commentary_audio)
            crowd_len = len(crowd_wave)

            if crowd_len < commentary_len:
                # Pad crowd audio
                padded_crowd = np.zeros(commentary_len, dtype=np.float32)
                padded_crowd[:crowd_len] = crowd_wave
                crowd_audio_matched = padded_crowd
            else:
                # Trim crowd audio
                crowd_audio_matched = crowd_wave[:commentary_len]

            # Mix with commentary louder than crowd
            mixed = commentary_audio * 0.85 + crowd_audio_matched * 0.15

            # Normalize to prevent clipping
            return safe_normalize(mixed)
